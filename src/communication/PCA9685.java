...
public class PCA9685 {
	...
	private double rayonMoteur = 0.00226;
	private double exposant = -3;

	private static double nombre = 10;

	private double resultat = Math.pow( nombre, exposant ); 
	...
	public PCA9685(int address) throws I2CFactory.UnsupportedBusNumberException {
		try {
			...
			if (verbose) {
				console.println("Connected to bus.... OK");
				info += "Connected to bus. OK.";
			}

			// Get the device itself
			servoDriver = bus.getDevice(address);
			if (verbose) {
				console.println("Connected to Device... OK");
				info += "Connected to device. OK.";
			}
			// Reseting
	...
	public void writeI2c(byte data) {
		try {
			this.servoDriver.write(data);
			console.println("writing on I2C bus...");
		} catch (IOException e) {
			console.println("Error writing on I2C bus...");
			info += "Unable to write on I2C servoDriver Bus !!!!";
			e.printStackTrace();
		}
	}
